...
class Tarifas_temporales_model extends CI_Model {
	...
	{
		$entrada	= date('Y-m-d', strtotime($consulta['entrada']));
		$salida		= date('Y-m-d', strtotime($consulta['salida']));
		...
		foreach ($habitaciones as $habitacion) 
		{
			...
			$sql = "SELECT 
					...
					WHERE 
						...
						AND 
							DATE_FORMAT(tarifas_temporales.entrada, '%d-%m-%Y') <= '$consulta[salida]')
						AND 
			...
			$sql = "SELECT 
					...
					INNER JOIN 
						tipos_tarifa ON(tarifa_habitacion.id_tarifa_temporal=tarifas_temporales.id_tarifa_temporal)
					WHERE 
						 (tarifas_temporales.salida  >= '$entrada' 
						AND 
							tarifas_temporales.entrada <= '$salida')
						AND 
							tarifa_habitacion.id_habitacion = '$habitacion->id_habitacion' 
					GROUP BY 
						tarifa_habitacion.id_habitacion
