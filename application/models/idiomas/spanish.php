<?php
	/* formulario de reserva */
	$texto['consulta'] 				= 'Consulta';
	$texto['reserva_online'] 		= 'Reserva online';
	$texto['entrada'] 				= 'Entrada';
	$texto['salida'] 				= 'Salida';
	$texto['ocupacion'] 			= 'Ocupación';
	$texto['adulto'] 				= 'Adulto';
	$texto['adultos'] 				= 'Adultos';
	$texto['sin_menores'] 			= 'sin menores';
	$texto['menor'] 				= 'Menor';
	$texto['menores'] 				= 'Menores';
	$texto['hotel'] 				= 'Hotel';
	$texto['monedas'] 				= 'Monedas';
	$texto['galeria'] 				= 'Galería';
	$texto['promociones'] 			= 'Promociones';
	
	/* reserva habitacions */
	$texto['leer_mas'] 				= 'Leer más';
	$texto['noches'] 				= 'Noches';
	$texto['seleccione_habitacion']	= 'Seleccione su habitación';
	$texto['habitacion']			= 'Habitación';
	$texto['habitaciones']			= 'Habitaciones';
	$texto['ver_fotos']				= 'Ver fotos';
	$texto['maximo_adultos']		= 'Máximo de adultos';
	$texto['maximo_menores']		= 'Máximo de menores';
	$texto['cantidad']				= 'Cantidad';
	$texto['sin_disponibilidad']	= 'sin disponibilidad';
	$texto['idiomas']				= 'Idiomas';
	$texto['no_habitaciones']		= 'No hay habitaciones disponibles';
	$texto['otras_opciones']		= 'Otras opciones disponibles';	
	
	/* reserva datos personales */
	$texto['datos_personales']		= 'Datos personales';
	$texto['nombre']				= 'Nombre';
	$texto['apellido']				= 'Apellido';
	$texto['email']					= 'Email';
	$texto['telefono']				= 'Teléfono';
	$texto['tipo_tarjeta']			= 'Tipo tarjeta';
	$texto['tarjeta']				= 'Tarjeta';
	$texto['agregar_nota']			= 'Agregar nota';
	$texto['ingrese']				= 'Ingrese';
	$texto['ejemplo'] 				= 'Ejemplo';
	$texto['nota'] 					= 'Nota';
	$texto['acepto_condiciones'] 	= 'Acepto las condiciones de la reserva.';
	$texto['ver_condiciones'] 		= 'Ver condiciones';
	$texto['seleccione_tarjeta']	= 'Seleccione una tarjeta';
	$texto['terminos_condiciones']	= 'Términos y condiciones';
	$texto['cerrar']				= 'Cerrar';
	$texto['pin']					= 'Pin';
	$texto['vencimiento']			= 'Vencimiento';
	$texto['vuelo']					= 'Vuelo';
	$texto['nro_de_vuelo']			= 'Número de vuelo';
	$texto['horario_llegada']		= 'Horario de llegada';	
	$texto['aerolinea']				= 'Aerolínea';
	$texto['datos']					= 'Datos';
	
	
	/* consultas */
	$texto['mensaje']				= 'Mensaje';
	$texto['enviar_consulta']		= 'Enviar consulta';
	$texto['direccion']				= 'Dirección';
	$texto['arriba']				= 'Ir arriba';
	$texto['favoritos']				= 'Agregar a favoritos';
		
	/* pasos */
	$texto['fecha']					= 'Fecha';
	$texto['confirmacion']			= 'Confirmación';
	
	/* categoria articulo*/
	$texto['fecha_publicacion']		= 'Fecha de publicación';
	$texto['volver']				= 'Volver';
	$texto['reservar']				= 'Reservar';
	
	/* habitacion descripcion*/
	$texto['descripcion']			= 'Descripción';
	$texto['servicios']				= 'Servicios';
	$texto['condiciones'] 			= 'Condiciones';
	$texto['como_llegar']			= 'Cómo llegar';
	$texto['calle']					= 'Calle';
	$texto['numero']				= 'Número';
	$texto['provincia']				= 'Provincia';
	$texto['medio']					= 'Medio';
	$texto['coche']					= 'Coche';
	$texto['transporte_publico']	= 'Transporte público';
	$texto['pie']					= 'A pie';
	$texto['bicicleta']				= 'Bicicleta';
	$texto['destino']				= 'Destino';
	$texto['desde']					= 'Desde';
	$texto['puntos_intermedios']	= 'Puntos intermedios';
	$texto['vehiculo_consumo']		= 'Consumo vehículo';
	$texto['precio_combustible']	= 'Precio combustible';
	$texto['ruta']					= 'Ruta';
	$texto['litros_kilometros']		= 'Listros x km';
	
	/* consulta */
	$texto['gracias_mensaje'] 		= 'Gracias por tu mensaje';
	$texto['en_breve'] 				= 'En breve nos comunicaremos con usted.';
	$texto['envio_habitacion']		= 'La habitación ha sido enviada.';
	$texto['su']					= 'su';
	$texto['ayuda_pin']				= '¡Sin gastos de transaccion! Su tarjeta de Crédito solo es necesaria para garantiza su reserva. 
   Los datos de tarjeta de credito dejados en garantía deberán ser válidos para confirmar su reserva, caso contrario la misma podrá ser dada de baja sin previo aviso. Dicha tarjeta es solo en concepto de garantía de reserva y será utilizada únicamente en caso de no-show o anulación fuera de termino, el monto total deberá ser abonado en el hotel.';
	
?>	